...
export class StatsService {
  ...
  private activeUsers = new Map<string, number>(); // userId or IP -> lastSeen timestamp
  private readonly ONLINE_THRESHOLD = 5 * 60 * 1000; // 5 minutes

  constructor(private prisma: PrismaService) {}

  /**
   * Record a user activity to track "Online Now" status
   */
  ...
  /**
   * Get current online count and total visits
   */
  async getStats() {
    const now = Date.now();
    ...
    // Get total visits from settings
