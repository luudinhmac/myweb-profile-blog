...
*   **Ngôn ngữ lập trình chính:** 
    ...
    *   *Xác minh:* [backend/tsconfig.json](file:///d:/DATA/Portfolio/backend/tsconfig.json) và [frontend/tsconfig.json](file:///d:/DATA/Portfolio/frontend/tsconfig.json).
*   **Môi trường Runtime & Quản lý Package:**
    ...
    *   *Xác minh:* [backend/Dockerfile:4](file:///d:/DATA/Portfolio/backend/Dockerfile#L4), [frontend/Dockerfile:2](file:///d:/DATA/Portfolio/frontend/Dockerfile#L2) và [local-development.md:10](file:///d:/DATA/Portfolio/scratch/infra-repo/docs/local-development.md#L10).
*   **Framework chính:**
    *   **Backend:** **NestJS v11.0.1** (sử dụng nền tảng Express bên dưới).
        *   *Xác minh:* Dependencies `@nestjs/core` và `@nestjs/common` tại [backend/package.json:26-28](file:///d:/DATA/Portfolio/backend/package.json#L26-L28).
    *   **Frontend:** **Next.js v16.2.6** (sử dụng React v19.2.4). Web UI được biên dịch ở chế độ *Standalone* để chạy độc lập qua Node server mà không cần toàn bộ mã nguồn dev.
        *   *Xác minh:* [frontend/package.json:18-21](file:///d:/DATA/Portfolio/frontend/package.json#L18-L21) và [frontend/Dockerfile:42](file:///d:/DATA/Portfolio/frontend/Dockerfile#L42).
*   **Các thư viện và gói phụ thuộc quan trọng nhất:**
    *   **Backend:**
        *   *ORM:* **Prisma Client v6.19.3** kết nối PostgreSQL (xem [backend/package.json:36](file:///d:/DATA/Portfolio/backend/package.json#L36)).
        *   *Database Driver:* `pg v8.13.1` (xem [backend/package.json:50](file:///d:/DATA/Portfolio/backend/package.json#L50)).
        *   *Caching:* `@nestjs/cache-manager v3.0.0` phối hợp với `cache-manager-redis-yet v5.0.0` (xem [backend/package.json:25,39](file:///d:/DATA/Portfolio/backend/package.json#L25) và [backend/package.json#L39](file:///d:/DATA/Portfolio/backend/package.json#L39)).
        *   *Storage Integration:* `minio v8.0.7` tương tác với các hệ thống S3-compatible (xem [backend/package.json:45](file:///d:/DATA/Portfolio/backend/package.json#L45)).
    ...
    *   **Frontend:**
        *   *HTTP Client:* `axios v1.15.2` phục vụ giao tiếp API (xem [frontend/package.json:13](file:///d:/DATA/Portfolio/frontend/package.json#L13)).
        *   *Styling:* `tailwindcss v4` kết hợp cùng PostCSS (xem [frontend/package.json:15,35](file:///d:/DATA/Portfolio/frontend/package.json#L15) và [frontend/package.json#L35](file:///d:/DATA/Portfolio/frontend/package.json#L35)).
        *   *Animation:* `framer-motion v12.38.0` (xem [frontend/package.json:15](file:///d:/DATA/Portfolio/frontend/package.json#L15)).
        *   *WYSIWYG Editor:* `react-quill-new v3.8.3` hỗ trợ soạn thảo bài viết (xem [frontend/package.json:23](file:///d:/DATA/Portfolio/frontend/package.json#L23)).
...
*   **Cơ sở dữ liệu chính (PostgreSQL):**
    *   **Loại triển khai:** StatefulSet trong Kubernetes nhằm giữ cố định định danh mạng (DNS) và duy trì ánh xạ vùng lưu trữ ổ đĩa lâu dài.
        *   *Xác minh:* [postgres/templates/statefulset.yaml](file:///d:/DATA/Portfolio/infra/apps/postgres/templates/statefulset.yaml).
    *   **Phiên bản cụ thể:** **PostgreSQL 16.13-alpine** (chạy trên môi trường container gọn nhẹ).
        *   *Xác minh:* [postgres/values.yaml:9](file:///d:/DATA/Portfolio/infra/apps/postgres/values.yaml#L9).
*   **Cơ sở dữ liệu bộ nhớ đệm (Redis):**
    ...
    *   **Phiên bản:** Sử dụng chart **Bitnami Redis (targetRevision: 19.1.3)** với image tag `latest`.
        *   *Xác minh:* [redis-prod.yaml:9-17](file:///d:/DATA/Portfolio/infra/argocd/applications/apps/redis-prod.yaml#L9-L17).
*   **Cơ chế cache:**
    ...
    *   *Xác minh:* [deployment.yaml:85-88](file:///d:/DATA/Portfolio/infra/apps/backend/templates/deployment.yaml#L85-L88) và [minio.storage.ts](file:///d:/DATA/Portfolio/backend/src/infrastructure/storage/minio.storage.ts).
...
*   **Local Storage (Lưu trữ cục bộ):**
    ...
    *   *Xác minh:* [infra-services/tasks/main.yml:153-158](file:///d:/DATA/Portfolio/infra/ansible/roles/infra-services/tasks/main.yml#L153-L158) and [postgres/values.yaml:19](file:///d:/DATA/Portfolio/infra/apps/postgres/values.yaml#L19).
*   **Persistent Volumes (PV) & StorageClass:**
    *   **Network File System / Block Storage:** Sử dụng giải pháp lưu trữ phân tán **Longhorn v1.7.0** làm hạ tầng lưu trữ khối chính cho Production.
        *   *Xác minh:* [longhorn.yaml:11-37](file:///d:/DATA/Portfolio/infra/argocd/applications/platform/longhorn.yaml#L11-L37).
    *   Trong môi trường Production, StorageClass `longhorn` (cấu hình replica = 1 do giới hạn hạ tầng VPS đơn) được gán cho các PVC cốt lõi:
        *   PostgreSQL Production Database: Kích thước 20Gi (xem [postgres-values.yaml:6-7](file:///d:/DATA/Portfolio/infra/environments/production/postgres-values.yaml#L6-L7)).
        *   Backend Upload Folder: Kích thước được cấp phát động (xem [backend-values.yaml:37](file:///d:/DATA/Portfolio/infra/environments/production/backend-values.yaml#L37)).
        *   Redis Master Persistence: Kích thước 2Gi (xem [redis-prod.yaml:30-33](file:///d:/DATA/Portfolio/infra/argocd/applications/apps/redis-prod.yaml#L30-L33)).
...
*   **Object Storage (Lưu trữ đối tượng):**
    ...
    *   *Mục đích ứng dụng:*
        1.  **Backend Media Storage:** Quản lý lưu trữ hình ảnh, tài liệu tĩnh của trang blog. Backend kết nối qua thư viện MinIO SDK sử dụng API key của Cloudflare R2 (xem [minio.storage.ts](file:///d:/DATA/Portfolio/backend/src/infrastructure/storage/minio.storage.ts) và các khoá mã hoá `MINIO_` trong SealedSecret [backend-values.yaml:43-49](file:///d:/DATA/Portfolio/infra/environments/production/backend-values.yaml#L43-L49)).
        2.  **Velero Backup Repository:** Làm nơi lưu trữ trung tâm của các bản sao lưu cụm K8s (bucket `velero-k8s-prod`, xem [velero.yaml:19-23](file:///d:/DATA/Portfolio/infra/argocd/applications/platform/velero.yaml#L19-L23)).
        3.  **etcd Snapshot Registry:** Nơi lưu trữ các tệp sao lưu dữ liệu cụm etcd hàng ngày (xem [cronjob.yaml:78](file:///d:/DATA/Portfolio/infra/platform/etcd-backup/cronjob.yaml#L78)).
*   **Volume Claims (PVC):**
    *   `portfolio-backend-production-pvc`: Liên kết ổ đĩa Longhorn vào đường dẫn `/app/uploads` và `/uploads` trong container backend để lưu tạm dữ liệu trước khi đẩy lên CDN hoặc phục vụ xử lý cục bộ (xem [pvc.yaml](file:///d:/DATA/Portfolio/infra/apps/backend/templates/pvc.yaml) và [deployment.yaml:108](file:///d:/DATA/Portfolio/infra/apps/backend/templates/deployment.yaml#L108)).
    *   `postgres-production-pvc`: Cấp phát cho thư mục lưu trữ dữ liệu gốc `/var/lib/postgresql/data` (xem [postgres/templates/pvc.yaml](file:///d:/DATA/Portfolio/infra/apps/postgres/templates/pvc.yaml)).
...
*   **Nền tảng hạ tầng:**
    ...
    *   *Xác minh:* [prod.ini:6](file:///d:/DATA/Portfolio/infra/ansible/inventory/prod.ini#L6) và [staging.ini:9](file:///d:/DATA/Portfolio/infra/ansible/inventory/staging.ini#L9).
*   **Điều phối container (Container Orchestration):**
    *   Cụm **Kubernetes** tự quản lý được cài đặt bằng công cụ kubeadm thông qua Ansible role `k8s-cluster` (xem playbook [k8s_setup.yml](file:///d:/DATA/Portfolio/infra/ansible/playbooks/k8s_setup.yml) và các task [k8s-cluster/tasks/init.yml](file:///d:/DATA/Portfolio/infra/roles/k8s-cluster/tasks/init.yml)).
*   **Các dịch vụ Kubernetes then chốt:**
    *   **Ingress Controller (Traefik):**
        ...
        *   *Xác minh:* [infra-services/tasks/main.yml:20-91](file:///d:/DATA/Portfolio/infra/ansible/roles/infra-services/tasks/main.yml#L20-L91).
    *   **Quản lý Bí mật (Bitnami Sealed Secrets):**
        ...
        *   *Xác minh:* [infra-services/tasks/main.yml:127-132](file:///d:/DATA/Portfolio/infra/ansible/roles/infra-services/tasks/main.yml#L127-L132) và tệp chứng chỉ mã hóa [sealed-cert.pem](file:///d:/DATA/Portfolio/infra/sealed-cert.pem).
    *   **Giám sát & Cảnh báo (Prometheus & Grafana):**
        ...
        *   *Xác minh:* [kube-prometheus-stack.yaml](file:///d:/DATA/Portfolio/infra/argocd/applications/monitoring/kube-prometheus-stack.yaml), [postgres-exporter.yaml](file:///d:/DATA/Portfolio/infra/monitoring/exporters/postgres-exporter.yaml) và [postgres.yaml](file:///d:/DATA/Portfolio/infra/monitoring/servicemonitors/postgres.yaml).
    *   **Giám sát/Logs:**
    ...
    *   **Tự động mở rộng (Autoscaling - HPA):**
        ...
        *   *Xác minh:* [infra-services/tasks/main.yml:94-102](file:///d:/DATA/Portfolio/infra/ansible/roles/infra-services/tasks/main.yml#L94-L102), [backend-values.yaml:32](file:///d:/DATA/Portfolio/infra/environments/production/backend-values.yaml#L32) và [frontend-values.yaml:18-22](file:///d:/DATA/Portfolio/infra/environments/production/frontend-values.yaml#L18-L22).
    ...
    *   **Backup cụm (Velero):**
        ...
        *   *Xác minh:* [velero.yaml](file:///d:/DATA/Portfolio/infra/argocd/applications/platform/velero.yaml) và [schedule.yaml](file:///d:/DATA/Portfolio/infra/platform/velero/schedule.yaml).
    *   **Sao lưu etcd (etcd-backup):**
        ...
        *   *Xác minh:* [etcd-backup/cronjob.yaml](file:///d:/DATA/Portfolio/infra/platform/etcd-backup/cronjob.yaml).
...
*   **CI/CD Pipeline (GitLab CI):**
    ...
    *   *Xác minh:* [backend/.gitlab-ci.yml](file:///d:/DATA/Portfolio/backend/.gitlab-ci.yml) and [frontend/.gitlab-ci.yml](file:///d:/DATA/Portfolio/frontend/.gitlab-ci.yml).
*   **Chiến lược Build & Deploy (GitOps):**
    ...
    *   *Xác minh:* [backend/.gitlab-ci.yml:258,346,385,424](file:///d:/DATA/Portfolio/backend/.gitlab-ci.yml#L258) và [infra/.gitlab-ci.yml](file:///d:/DATA/Portfolio/infra/.gitlab-ci.yml).
...
*   **HTTPS/SSL & Quản lý Chứng chỉ:**
    ...
    *   *Xác minh:* [cluster-issuer.yaml](file:///d:/DATA/Portfolio/infra/platform/cert-manager/cluster-issuers/cluster-issuer.yaml) và cấu hình tls trong [ingress.yaml:12-23](file:///d:/DATA/Portfolio/infra/apps/frontend/templates/ingress.yaml#L12-L23).
*   **Tường lửa & Chính sách mạng:**
    *   **Host Firewall (UFW):** Cấu hình trực tiếp trên hệ điều hành VPS thông qua Ansible. Thiết lập mặc định chặn toàn bộ kết nối đi vào (Default Deny), chỉ mở các cổng dịch vụ thiết yếu: 80 (HTTP), 443 (HTTPS), 2222 (cổng SSH bảo mật đã đổi từ cổng 22 mặc định), dải IP mạng nội bộ của Kubernetes Pods và Services.
        *   *Xác minh:* [firewall.yml](file:///d:/DATA/Portfolio/infra/ansible/roles/security/tasks/firewall.yml).
    *   **Hệ thống mạng cụm (CNI):** Sử dụng giải pháp **Cilium v1.16.1** (được cài đặt qua Helm). Cilium thay thế hoàn toàn kube-proxy truyền thống bằng công nghệ eBPF hiệu năng cao, tích hợp sẵn Hubble để giám sát và trực quan hóa luồng dữ liệu mạng.
        *   *Xác minh:* [k8s-cluster/tasks/init.yml:50-57](file:///d:/DATA/Portfolio/infra/ansible/roles/k8s-cluster/tasks/init.yml#L50-L57).
    *   **Kubernetes Network Policy:** Áp dụng chính sách mặc định `default-deny-ingress` trong namespace `portfolio` để chặn toàn bộ lưu lượng Ingress không mong muốn đi vào các Pod ứng dụng trừ khi có cấu hình cụ thể cho phép.
        *   *Xác minh:* [k8s-cluster/tasks/hardening.yml:10-20](file:///d:/DATA/Portfolio/infra/ansible/roles/k8s-cluster/tasks/hardening.yml#L10-L20).
*   **Xác thực & Phân quyền (Authentication & Authorization):**
    *   *Key/Token:* Backend NestJS tự triển khai cơ chế cấp phát và xác minh mã Token bảo mật thông qua **JWT (JSON Web Tokens)** sử dụng thư viện `@nestjs/jwt` kết hợp Passport.js. Khoá bí mật dùng để ký token được mã hóa và truyền vào container dưới dạng biến môi trường `JWT_SECRET`.
        *   *Xác minh:* [backend/package.json:29,48-49](file:///d:/DATA/Portfolio/backend/package.json#L29) và [local-development.md:51](file:///d:/DATA/Portfolio/scratch/infra-repo/docs/local-development.md#L51).
