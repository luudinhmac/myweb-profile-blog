...
function RegisterContent({ siteKey }: { siteKey: string }) {
  ...
  useEffect(() => {
    ...
    const siteKeyToUse = siteKey || '1x00000000000000000000AA';

    loadTurnstile().then(() => {
      const turnstile = (window as any).turnstile;
      if (turnstile && document.getElementById('turnstile-container')) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await authService.register(formData, turnstileToken);
      ...
      <div className="max-w-xl w-full relative z-10">
        ...
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-6 md:p-8 rounded-[2.5rem] border border-slate-200/50 dark:border-slate-800/50 shadow-2xl relative overflow-hidden ring-1 ring-black/5">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div id="turnstile-container" className="my-4 min-h-[65px] flex justify-center scale-90"></div>
            ...
            <div className="pt-3">
              <Button
                ...
                className="w-full py-3 text-xs font-black uppercase tracking-[0.2em] rounded-xl shadow-lg shadow-primary/20 bg-gradient-to-r from-primary to-blue-600 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Đăng ký
              </Button>
            </div>
...
export default function RegisterClient({ siteKey }: { siteKey: string }) {
  return <RegisterContent siteKey={siteKey} />;
}
