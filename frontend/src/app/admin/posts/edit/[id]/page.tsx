...
export default function EditPostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [postRes, catsRes, seriesRes] = await Promise.all([
  ...
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 p-6">
         <div className="bg-white dark:bg-slate-900 p-8 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xl max-w-md w-full text-center">
            <AlertCircle size={48} className="mx-auto text-red-500 mb-4" />
            <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Truy cập bị chặn</h2>
            <p className="text-slate-500 mb-6">{error}</p>
            <Link href="/admin" className="inline-block px-6 py-3 bg-primary text-white rounded-xl font-bold shadow-lg shadow-primary/20">Quay lại Dashboard</Link>
         </div>
      </div>
    );
  }
                   ...
                   <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="text" placeholder="tag1, tag2..." value={formData.tags} onChange={(e) => setFormData({ ...formData, tags: e.target.value })}
                          className="w-full pl-9 pr-3 py-2.5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-xs outline-none focus:ring-2 focus:ring-primary" />
                      </div>
                   </div>
                   <div>
